...
def parse_comparison(value):
    """Parse the comparison input and return a function for numerical comparison."""
...
def find_pokemon_by_attribute(pokemontxt, attribute, value):
    """Find Pokémon by numerical or categorical attributes."""
...
def filter_pokemon_by_any(pokemontxt, filters):
    ...
    for pokemon in pokemontxt:
        for attr, values in filters:
            ...
            if any(val in values for val in values):
                result.append(pokemon)
                break  # No need to check other filters for this Pokémon
    return result
    ...
    while True:
        ...
        if initialInput == "categories":
            ...
            while True:
                ...
                # Enter filtered list loop
                last_action = 'initial'
                while True:
                    if last_action == 'initial':
                        print(f"\nAll Pokémon wherein the value of the '{attribute}' criteria is '{value}':")
                        for pokemon in pokemontxt:
                    ...
                    if action in ("cancel", "exit", "quit", "stop"):
                        ...
                        # Apply the new filter and get the updated list of Pokémon
                        pokemontxt = filter_pokemon_by_any(all_pokemontxt, filters)
                        pokemontxt = sort_pokemon_by_order(pokemontxt, order_list)
                        ...
                        # Apply the filters and get the updated list of Pokémon
                        pokemontxt = filter_pokemon_by_all(all_pokemontxt, filters)
                        pokemontxt = sort_pokemon_by_order(pokemontxt, order_list)

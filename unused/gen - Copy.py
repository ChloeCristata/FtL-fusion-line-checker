...
# import csv
# 
# SPRITEPACK_SOURCE = "customSprites.csv"
# 
# def sprite_exists(head, body):
#     spriteExists = set()
#     with open(SPRITEPACK_SOURCE, newline='', encoding='utf-8') as csvfile:
#         reader = csv.reader(csvfile)
#         for row in reader:
#             fusion_id = row[0]
#             print(f"Checking fusion_id: {fusion_id}")  # Print the fusion_id being processed
#             parts = fusion_id.split('.')
#             if len(parts) == 2:
#                 row_head, row_body = parts
#                 try:
#                     if head == int(row_head) and body == int(row_body):
#                         spriteExists.add(fusion_id)
#                 except ValueError:
#                     continue  # Skip this row if conversion to int fails
#     return spriteExists
# 
# customSpriteList = []
# 
